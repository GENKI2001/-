...
import {
  ...
  useDotButton,
  usePrevNextButtons,
} from './buttons/Carousel.DotButton.Atoms';
...
export const FlexCarousel: React.FC<Props> = ({
  isBanner,
  displayButton,
  playOnInit,
  items,
  ...
  const ModifiedPrevButton = () => {
    if (React.isValidElement(prevButton)) {
      return React.cloneElement(
        prevButton as React.ReactElement<{ onClick?: () => void }>,
        {
          onClick: () => {
  ...
  const ModifiedNextButton = () => {
    if (React.isValidElement(nextButton)) {
      return React.cloneElement(
        nextButton as React.ReactElement<{ onClick?: () => void }>,
        {
          onClick: () => {
  ...
  return (
    <div
      className={isBanner ? 'banner' : 'embla'}
      style={
        {
          position: 'relative',
    ...
    >
      <div
        className={isBanner ? 'banner__viewport' : 'embla__viewport'}
        ref={emblaRef}
      >
        <div className={isBanner ? 'banner__container' : 'embla__container'}>
          {(items ?? []).map((item: any, index) => (
            <div
              className={isBanner ? 'banner__slide' : 'embla__slide'}
              key={index}
              style={{
            ...
            >
              <div
                className={
                  isBanner ? 'banner__slide__number' : 'embla__slide__number'
                }
              >
                {item}
              </div>
            ...
            {scrollSnaps.map((_, index) => {
              return (
                <BannerCarouselDotButton
                  ...
                  style={{
                    backgroundColor:
                      index === selectedIndex ? '#383E86' : '#C2C2C2', // 選択された場合のスタイル
